...
for (let i = 0; i <c; i++) { // loop through the original number of rows 
  for (let j=0; j< d; j++) { // loop through the new number of columns
...
for (let i=0; i< d; i++) { // loop through entire new array
  for (let j=0; j<d; j++) { // loop through sub-array
    if (input[i][j] > 9) { // if any number is greater than 9, subtract 9 from it
...
const checkNeighbors = (i, j, count) => {
  ...
  if (i === d - 1 && j === d - 1) {
    if (count < total) {
      ...
      // or it will run before my algorithm(?) finishes
    }
    return;
  }
  ...
  if (input[i][j + 1] > 0) {
    setTimeout(() => { // added setTimeouts so the parent functions could complete.
...
// This code took a long time to run... Probably ~10 minutes
